...
MODEL_NAME = "turboparkgauss"              # "jensen", "gauss", "curl", "turboparkgauss"
N_SAMPLES  = 1000                          # number of datapoints to generate
ENV_FILE   = "configs/env.yaml"            # path to your YAML file
...
def sample_env_value(entry: dict) -> float:
    """Samples scalar from YAML entry {fixed: value} or {min: lo, max: hi}."""
    if "fixed" in entry:
...
def generate_layout(num_turbines: int, spacing: float = 500.0):
    """Simple 1D turbine layout (x-axis row)."""
...
# ---------------------------------------------------------------------
# VISUALIZATION — combined subplot figure
# ---------------------------------------------------------------------

def plot_samples(samples, model):
    """
    Create a single figure with multiple wake plots.
    Uses FLORIS 4.5 API:
       wf = fm.calculate_wake()
       hor = wf.get_hor_plane(z=...)
    """
    fm = model.fm
    ...
    for ax_idx, (ax, s) in enumerate(zip(axes, samples)):
        ...
        # --- FLORIS 4.5: compute wake field ---
        wf = fm.calculate_wake()
        hor = wf.get_hor_plane(z=90.0)

        X = hor.x
        Y = hor.y
        U = hor.u

        c = ax.contourf(X, Y, U, levels=40)
        ax.set_title(
            f"Sample {idx + 1}\n"
...
def run_sweep(model_name: str, N: int, env: dict):
    ...
    # Indices to visualize: 1, 100, 200, ... <= N
    ...
    # --- 7) combined subplot visualization ---
    plot_samples(vis_samples, model)
